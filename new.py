...
while cap.isOpened():
    ...
    img_sub=cv2.subtract(frame_gray,background) 
    dilatada=img_sub
    
    dilate=cv2.dilate(img_sub,np.ones((3,3)))
    kernel=cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5))
    dilatada=cv2.morphologyEx(dilate,cv2.MORPH_CLOSE,kernel,iterations=2)
    cv2.imshow('frame',dilatada)
    counterShape,h=cv2.findContours(dilatada,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)

    cv2.line(frame,(0,count_line_postion),(1500,count_line_postion),(255,127,0),3)
    ...
    for (i,c) in enumerate(counterShape):
        ...
        cv2.putText(frame,'Vehicle'+str(counter),(x,y-20),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
    ...
    cv2.putText(frame,'Vehicle Counter:'+str(counter),(10,50),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)  
